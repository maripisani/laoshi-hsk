...
// ══════════════════════════════════════════════════════════════
// 老师 (Laoshi) · VOCABULÁRIO COMPLETO NOVO HSK 3.0
// Todos os níveis: HSK 1–9 · GF0025-2021 / Revisão 2025-11
// ══════════════════════════════════════════════════════════════

// Formato: "汉字|pīnyīn|pos|nível" por linha
const RAW = `
爱|ài|动|1
八|bā|数|1
爸爸|bàba|名|1
...
function WordCard({ item, showPy, translation, loading }) {
  const cfg = LEVEL_CFG[item.l];
  const pt = translation || (item.l <= 3 ? (TR13[item.h] || "—") : undefined);
  const posLabel = POS_PT[item.pos] || item.pos;
  return (
    <div style={{
      background:"#fff", border:"1px solid #e2e8f0", borderRadius:"12px",
      padding:"12px 14px", display:"flex", flexDirection:"column", gap:"5px",
      borderLeft:`4px solid ${cfg.badge}`, cursor:"default",
      boxShadow:"0 1px 2px rgba(0,0,0,0.04)", transition:"transform .15s, box-shadow .15s",
      minHeight:"108px",
    }}
    ...
    >
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
        <span style={{fontSize:"1.6rem",fontWeight:"700",color:"#0f172a",lineHeight:1.1,fontFamily:"serif"}}>{item.h}</span>
        <span style={{fontSize:"0.6rem",fontWeight:"700",color:cfg.badge,background:cfg.bg,
          border:`1px solid ${cfg.badge}30`,borderRadius:"5px",padding:"2px 6px",marginTop:"2px",whiteSpace:"nowrap"
        }}>{cfg.label}</span>
      </div>
      {showPy && <Pinyin py={item.py} />}
      <span style={{fontSize:"0.65rem",color:"#94a3b8",background:"#f8fafc",borderRadius:"4px",padding:"1px 5px",alignSelf:"flex-start"}}>{posLabel}</span>
      {loading ? (
        <span style={{fontSize:"0.78rem",color:"#94a3b8",fontStyle:"italic"}}>traduzindo…</span>
      ) : pt ? (
        <span style={{fontSize:"0.82rem",color:"#334155",fontWeight:"500",marginTop:"1px",lineHeight:1.3}}>{pt}</span>
      ) : (
        <span style={{fontSize:"0.78rem",color:"#cbd5e1",fontStyle:"italic"}}>—</span>
      )}
    </div>
  );
}
...
export default function LaoshiVocab() {
  ...
  const filtered = useMemo(() => {
    let list = VOCAB;
    if (level > 0) list = list.filter(v => v.l === level);
    if (search.trim()) {
      const q = search.trim().toLowerCase();
      list = list.filter(v => v.h.includes(q) || v.py.toLowerCase().includes(q) || (TR13[v.h]||"").toLowerCase().includes(q) || (cache.get(v.h)||"").toLowerCase().includes(q));
    }
    return list;
  }, [level, search, cache]);
  ...
  // Auto-translate HSK 4+ when page changes
  useEffect(() => {
    const needsTranslation = pageItems.filter(v => v.l >= 4 && !cache.has(v.h));
    if (needsTranslation.length === 0 || translateRef.current) return;

    const doTranslate = async () => {
      translateRef.current = true;
      setTranslating(true);
      try {
        const batch = needsTranslation.map(v => `${v.h}(${v.py})`).join(", ");
        const prompt = `Você é um tradutor chinês-português. Traduza estas palavras chinesas para português brasileiro de forma concisa (máx. 6 palavras por tradução). Responda APENAS com JSON válido sem markdown, formato exato: {"t":[{"h":"hanzi","pt":"tradução"}]}. Palavras: ${batch}`;
        ...
        if (parsed.t) {
          setCache(prev => {
            ...
            parsed.t.forEach(({h,pt}) => next.set(h, pt));
            return next;
          });
        }
      } catch(e) {
  ...
  function selectLevel(l) { setLevel(l); setPage(1); setSearch(""); }

  const counts = useMemo(() => {
    const c = {};
    VOCAB.forEach(v => { c[v.l] = (c[v.l]||0) + 1; });
    return c;
  }, []);
  ...
  return (
    <div style={{fontFamily:"'Segoe UI',system-ui,sans-serif",background:"#f8fafc",minHeight:"100vh"}}>
      {/* HEADER */}
      <div style={{background:"linear-gradient(135deg,#0f172a,#1e3a5f 55%,#0f172a)",padding:"24px 20px 20px",textAlign:"center"}}>
        <div style={{fontSize:"0.7rem",color:"#94a3b8",letterSpacing:"0.15em",textTransform:"uppercase",marginBottom:"5px"}}>
          老师 · Laoshi · Novo HSK 3.0
        </div>
        <h1 style={{margin:0,color:"#f8fafc",fontSize:"clamp(1.2rem,4vw,1.75rem)",fontWeight:"800",letterSpacing:"-0.02em"}}>
          词汇大纲 · Vocabulário Completo HSK 1–9
        </h1>
        <p style={{margin:"6px 0 0",color:"#94a3b8",fontSize:"0.8rem"}}>
          {totalWords.toLocaleString()} vocábulos oficiais · GF0025-2021 · HSK 4–9 traduzidos por IA em tempo real
        </p>
        {/* Level stats row */}
        <div style={{display:"flex",justifyContent:"center",gap:"6px",marginTop:"14px",flexWrap:"wrap"}}>
          {[1,2,3,4,5,6,7].map(l => {
            ...
            return (
              <div key={l} style={{background:"rgba(255,255,255,0.07)",borderRadius:"8px",padding:"5px 12px",
                border:`1px solid ${cfg.badge}40`,cursor:"pointer"}}
                onClick={() => selectLevel(l)}>
                <span style={{color:cfg.badge,fontWeight:"700",fontSize:"1rem"}}>{counts[l]||0}</span>
                <span style={{color:"#94a3b8",fontSize:"0.68rem",marginLeft:"4px"}}>{cfg.label}</span>
              </div>
            );
          })}
        </div>
      ...
      <div style={{background:"#fff",borderBottom:"1px solid #e2e8f0",padding:"12px 16px",position:"sticky",top:0,zIndex:10}}>
        <div style={{maxWidth:"980px",margin:"0 auto",display:"flex",gap:"8px",flexWrap:"wrap",alignItems:"center"}}>
          {/* Level filter */}
            ...
            {[0,1,2,3,4,5,6,7].map(l => {
              ...
              return (
                <button key={l} onClick={() => selectLevel(l)} style={{
                  ...
                  fontWeight:"600",fontSize:"0.75rem",transition:"all .15s",
                  background: active ? cfg.badge : "#f1f5f9",
                  color: active ? "#fff" : "#64748B",
                  boxShadow: active ? `0 2px 6px ${cfg.badge}40` : "none",
                }}>
                  {cfg.label}
                </button>
              );
          ...
          <div style={{flex:1,minWidth:"150px",position:"relative"}}>
            <span style={{position:"absolute",left:"9px",top:"50%",transform:"translateY(-50%)",color:"#94a3b8",fontSize:"0.85rem"}}>🔍</span>
            <input value={search} onChange={e=>{setSearch(e.target.value);setPage(1);}}
              placeholder="Buscar hanzi, pinyin ou português..."
              style={{width:"100%",padding:"6px 10px 6px 28px",borderRadius:"16px",
                border:"1px solid #e2e8f0",fontSize:"0.8rem",outline:"none",background:"#f8fafc",boxSizing:"border-box"}} />
          </div>
          {/* Pinyin toggle */}
          <button onClick={() => setShowPy(p => !p)} style={{
            padding:"5px 12px",borderRadius:"16px",border:"1px solid #e2e8f0",cursor:"pointer",
            fontSize:"0.75rem",fontWeight:"600",
            background: showPy ? "#fef3c7" : "#f1f5f9",
            color: showPy ? "#92400e" : "#64748B",
          }}>
            {showPy ? "拼 Pinyin ✓" : "拼 Pinyin ✗"}
          </button>
        </div>
      </div>

      {/* STATUS BAR */}
      <div style={{maxWidth:"980px",margin:"10px auto 0",padding:"0 16px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <span style={{fontSize:"0.75rem",color:"#94a3b8"}}>
          {filtered.length.toLocaleString()} palavra{filtered.length !== 1 ? "s" : ""}
          {totalPages > 1 && ` · Página ${page}/${totalPages}`}
          {cachedCount > 0 && ` · ${cachedCount} traduz. em cache`}
        </span>
        {translating && (
          <span style={{fontSize:"0.73rem",color:"#7c3aed",fontStyle:"italic",animation:"pulse 1s infinite"}}>
            ⟳ Traduzindo com IA…
          </span>
        )}
        {level >= 4 && !translating && (
          <span style={{fontSize:"0.7rem",color:"#94a3b8"}}>
            HSK {level <= 6 ? level : "7–9"} · Traduções geradas por IA
          </span>
        )}
      </div>

      {/* CARDS */}
      <div style={{maxWidth:"980px",margin:"10px auto 16px",padding:"0 16px"}}>
        {pageItems.length === 0 ? (
          <div style={{textAlign:"center",padding:"60px 20px",color:"#94a3b8"}}>
            <div style={{fontSize:"3rem"}}>🔎</div>
            <div style={{marginTop:"10px",fontSize:"1rem"}}>Nenhuma palavra encontrada.</div>
          </div>
        ) : (
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(170px,1fr))",gap:"10px"}}>
            {pageItems.map((item, i) => (
              <WordCard key={item.h+item.l+i} item={item} showPy={showPy}
                translation={cache.get(item.h)}
                loading={translating && item.l >= 4 && !cache.has(item.h)} />
            ))}
          </div>
        )}
      </div>
      ...
      {/* PAGINATION */}
      {totalPages > 1 && (
        <div style={{display:"flex",justifyContent:"center",gap:"6px",padding:"12px 16px 28px",flexWrap:"wrap"}}>
          <PagBtn label="«" onClick={() => setPage(1)} disabled={page===1} />
          <PagBtn label="‹" onClick={() => setPage(p=>Math.max(1,p-1))} disabled={page===1} />
          {getPageRange(page, totalPages).map((p,i) => (
            <PagBtn key={i} label={p === "…" ? "…" : String(p)} onClick={() => p !== "…" && setPage(p)}
              active={p === page} disabled={p === "…"} />
          ))}
      ...
      {/* FOOTER */}
      <div style={{borderTop:"1px solid #e2e8f0",padding:"14px 20px",textAlign:"center",
        fontSize:"0.68rem",color:"#94a3b8",background:"#fff"}}>
        老师 (Laoshi) · Vocabulário Completo Novo HSK 3.0 · GF0025-2021 · {totalWords.toLocaleString()} palavras · HSK 1–3 com traduções embutidas · HSK 4–9 com IA
      </div>
...
function PagBtn({ label, onClick, disabled, active }) {
  return (
    <button onClick={onClick} disabled={disabled} style={{
      padding:"5px 10px",borderRadius:"7px",border:"1px solid #e2e8f0",cursor:disabled?"not-allowed":"pointer",
      minWidth:"32px",fontSize:"0.75rem",fontWeight:"600",transition:"all .1s",
